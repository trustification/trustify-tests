...
`PLAYWRIGHT_PORT` from `.devcontainer/.env`). You can override this value if it
is already taken by the system or other application:
```sh
export PLAYWRIGHT_PORT=<your choice of port number>
```

Then, start the Playwright service (you can override `.devcontainer/.env` by
exporting environment variables with your own values as demonstrated above or
you can just pass to `{docker,podman}-compose` your own `.env` file via
`--env-file <env_file>` CLI option):
```sh
podman-compose -f .devcontainer/compose.yaml up
```
...
and:
```sh
podman-compose -f .devcontainer/compose.yaml down
```
